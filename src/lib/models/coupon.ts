...
export interface ICoupon extends Document {
  ...
  description: string; // human-readable description shown to customers
  minOrderAmount: number; // minimum order amount in pounds
  maxUses: number;
  usedCount: number;
  expiresAt: Date | null;
  active: boolean;
  isPublic: boolean; // if true, shown on homepage; if false, only usable by direct code share
  createdAt: Date;
  updatedAt: Date;
}
...
CouponSchema.index({ isPublic: 1, active: 1 });
